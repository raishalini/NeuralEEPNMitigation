...
from plots import SNRVsLinewidthPlotterNN, MSEVsLinewidthPlotterNN, ScatterPlot, EVMVsLinewidthPlotterNN
...
class Transceiver:
    ...
    def cal_mse(self, x, y):
        ...
        Q_Y_values = (1 / (tf.math.sqrt(2 * np.pi) * mse)) * tf.math.exp(-(tf.math.square(tf.abs(x - y))) / (2 * tf.math.square(mse)))
        entropy_reg = -tf.reduce_mean(tf.math.log(Q_Y_values))

        mse_x = mse - 2 * (tf.math.square(mse) * entropy_reg)
        
        return mse, snr, evm, mse_x
    ...
    def train_and_test(self, iterations, linewidths, link_distances):
        ...
        for idx, lw in enumerate(linewidths):
            ...
            for length in link_distances:
                ...
                test_results[lw][length]["mse_x"].append({"original_mse_x": original_mse_x.numpy(), "nn_mse_x": nn_mse_x.numpy()})
        ...
        return test_results, tx_symbols_arr, rx_symbols_arr, rx_symbols_nn_arr
    ...
    def test(self, model, length, linewidth, num_symbols):
        ...
        return org_mse, org_snr, org_evm, org_mse_x, nn_mse, nn_snr, nn_evm, nn_mse_x, tx_symbols_arr, rx_symbols_arr, rx_symbols_nn_arr
...
test_results, tx_symbols_arr, rx_symbols_arr, rx_symbols_nn_arr = pipeline.train_and_test(iterations=iterations, linewidths = linewidths, link_distances=link_distances)
